...
local function SimulateParry(isSpam, ballSpeed)
    ...
    if isSpam then
        if ballSpeed and ballSpeed > 300 then
            minDelay = 0.010 -- Экстремально быстрые шары (уменьшено)
        elseif ballSpeed and ballSpeed > 250 then
            minDelay = 0.012 -- Очень быстрые шары (уменьшено)
        elseif ballSpeed and ballSpeed > 150 then
            minDelay = 0.014 -- Быстрые шары (уменьшено)
        else
            minDelay = 0.016 -- Обычные шары (уменьшено)
        end
    else
    ...
    for _, ball in pairs(balls:GetChildren()) do
        if ball:IsA("BasePart") then
            ...
            if targetName == LocalPlayer.Name and Settings.Enabled then
                ...
                -- Защита от повторного парирования того же шара (уменьшена с 0.18 до 0.12)
                if lastParryBall == ballId and (currentTime - lastParryBallTime) < 0.12 then
                    continue
                end
                
                -- Проверка по времени (минимальная задержка для мгновенной реакции)
                -- Для быстрых шаров используем еще меньшую задержку
                local checkDelay = ballSpeed > 250 and 0.015 or (ballSpeed > 150 and 0.02 or 0.025)
                ...
                if Settings.AutoSpam then
                    ...
                    if ballSpeed > 300 then
                        adaptiveSpamDist = Settings.SpamDistance + 20 -- Для безумно быстрых шаров
                        adaptiveSpamDelay = 0.012 -- Очень быстрый спам (уменьшено)
                    elseif ballSpeed > 250 then
                        adaptiveSpamDist = Settings.SpamDistance + 15 -- Для супер быстрых шаров
                        adaptiveSpamDelay = 0.015 -- Быстрый спам (уменьшено)
                    elseif ballSpeed > 200 then
                        adaptiveSpamDist = Settings.SpamDistance + 12 -- Для очень быстрых шаров
                        adaptiveSpamDelay = 0.018 -- Средне-быстрый спам (уменьшено)
                    elseif ballSpeed > 150 then
                        adaptiveSpamDist = Settings.SpamDistance + 9 -- Для быстрых шаров
                        adaptiveSpamDelay = 0.020 -- Средний спам (уменьшено)
                    elseif ballSpeed > 100 then
                        adaptiveSpamDist = Settings.SpamDistance + 6 -- Для средних шаров
                        adaptiveSpamDelay = 0.022 -- Базовый спам (уменьшено)
                    else
                        -- Для медленных шаров
                        adaptiveSpamDelay = 0.025 -- Медленный спам (уменьшено)
                    end
                ...
                -- Дополнительная проверка: если шар очень близко (критическая дистанция), парируем сразу
                ...
                if currentDistance <= criticalDistance or currentDistance <= idealDistance or isApproachingFast then
                    local reason = "IDEAL"
